...
float Ki = 0.3;
float Kd = 0;
bool pido = true, rflag = false, gomflag = false;
float ishowspeed = 0;
int ztick = 0, ltick = 0;
...
//      trans.setAppendStatus(0);    // When enabled, two status bytes will be appended to the payload of the packet. The status bytes contain RSSI and LQI values, as well as CRC OK.
  ...
  while (1)
  {
	  magn1.Read();
	  magn2.read();
	  float wx = gyro.X(), wy = gyro.Y(), wz = gyro.Z();
	  float mx1 = magn1.X(), my1 = magn1.Y(), mz1 = magn1.Z();
	  float mx2 = magn2.X(), my2 = magn2.Y(), mz2 = magn2.Z();

	  if (!rflag) HAL_UART_Receive(&huart1, (uint8_t*)&str, 1, 1000);
	  ...
	  if(pido == true) {
			Integrator += wz;
			if (Integrator > 5.0f) { Integrator = 5.0f; }
			if (Integrator < -5.0f) { Integrator = -5.0f; }
		    float P = Kp * wz;
		    float I = Integrator * Ki;
			float PID_rez = P + I;
			ishowspeed = motor.CurrentSpeed() + PID_rez;

			if (ishowspeed > 5000) ishowspeed = 5000;
			if (ishowspeed < -5000) ishowspeed = -5000;
			motor.NeedSpeed(ishowspeed);
	  }
	  //if(str == 'r' || gomflag == true) {
	  if(HAL_GetTick() - ltick > 20000) {
		  ltick = HAL_GetTick();
		  str = '\n';
		  if(abs(wz) <= 0.25) {
			  if (ishowspeed >= 0) motor.NeedSpeed(ishowspeed-1250);
			  else motor.NeedSpeed(ishowspeed+1250);
			  rflag = true;
			  pido = false;
			  gomflag = false;
		  } else {
	  ...
	  if (rflag == true) {
		  if(HAL_GetTick() - ztick > 10) {
			  ztick = HAL_GetTick();

			  mmu_data[mmu_current] = {
			  ...
			  if (mmu_current >= 360)
			  {
				  ...
				  for (int i = 0; i < 360; i++)
				  {
					  char buff[128] = {0};
					  MMU mmu = mmu_data[i];
					  int l488 = sprintf(buff, "\n\r%.10f, %.10f, %.10f, %.10f, %.10f, %.10f", mmu.x1, mmu.y1, mmu.z1, mmu.x2, mmu.y2, mmu.z2);
					  HAL_UART_Transmit(&huart1, (uint8_t*)buff, l488, 1000);
	  ...
	  /*if(HAL_GetTick() - ltick > 1000) {
		  ltick = HAL_GetTick();
		  char buf[32] = {0};
		  int dlen = sprintf(buf, "\n\rpido=%s\n\r", pido ? "True" : "False");
		  HAL_UART_Transmit(&huart1, (uint8_t*)buf, dlen, 1000);
	  }*/
    /* USER CODE END WHILE */
